...
func handle_shoot():
	if Input.is_action_just_pressed("shoot1"):
		...
		if isFlipped and not Input.is_action_pressed('up1') and not Input.is_action_pressed('down1'):
			muzzle.position = Vector2(-16, -8)
			b.set_speed(Vector2(-1, 0))
		elif not isFlipped and not Input.is_action_pressed('up1') and not Input.is_action_pressed('down1'):
			muzzle.position = Vector2(16, -8)
			b.set_speed(Vector2(1, 0))
